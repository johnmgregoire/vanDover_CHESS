...
def calcdiffcounts(h5path, h5groupstr, diffdict, savename='dcounts', savebool=True, plotbool=False, intbool=True, intplotbool=False, intplotallbool=False, icountsbool=True):
...
def chess2011integrate(h5path, h5groupstr, bckndbool=False):
    h5file=h5py.File(h5path, mode='r+')
    h5analysis=h5file['/'.join((h5groupstr, 'analysis'))]
    h5mar=h5file['/'.join((h5groupstr, 'analysis', getxrdname(h5analysis)))]
    h5marcounts=h5file['/'.join((h5groupstr,'measurement', getxrdname(h5analysis), 'counts'))]
    ...
    for pointind in pointlist:
        ...
        data=h5marcounts[pointind, :, :]
        if bckndbool:
            data=bckndsubtract(data, constructbckndarr_linbyposn(bckndarr, pointind), imap, btype='lin', linweights=blinwts[pointind])[0]
        icounts[pointind, :]=intbyarray(data, imap, dqchiimage, slots, mean=True)[:]
    h5file.close()
